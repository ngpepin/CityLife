...
const CITYLIFE_UTILITY_FREE_TYPES = new Set<BuildingType>([
  'grass',
  'empty',
  'water',
  'road',
...
function isUtilityFreeBuildingType(type: BuildingType): boolean {
  return !CITYLIFE_UTILITY_FREE_TYPES.has(type);
}

function forceInstantConstructedBuilding(building: Building): void {
...
function applyCityLifeBuildingOverrides(
  grid: Tile[][],
  x: number,
  y: number,
  width: number,
  ...
  for (let dy = 0; dy < height; dy++) {
    for (let dx = 0; dx < width; dx++) {
      ...
      if (!tile) continue;
      tile.zone = 'none';
      if (isUtilityFreeBuildingType(tile.building.type)) {
        forceInstantConstructedBuilding(tile.building);
      }
    }
  }
}
...
export function upgradeServiceBuilding(state: GameState, x: number, y: number): GameState | null {
  ...
  const hasUnlimitedBudget = isCityLifeState(state);
  
  // Check if player has enough money
  if (!hasUnlimitedBudget && state.stats.money < upgradeCost) return null;
  ...
  const newStats = {
    ...state.stats,
    money: hasUnlimitedBudget
      ? Math.max(state.stats.money, CITYLIFE_UNLIMITED_MONEY)
      : state.stats.money - upgradeCost,
  };
...
export function simulateTick(state: GameState): GameState {
  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      ...
      const cityLifeUtilityFreeBuilding = cityLifeMode && isUtilityFreeBuildingType(originalBuilding.type);
      const newPowered = cityLifeUtilityFreeBuilding ? true : services.power[y][x];
      const newWatered = cityLifeUtilityFreeBuilding ? true : services.water[y][x];
      ...
      if (tile.zone === 'none' &&
        ...
        if (cityLifeMode && isUtilityFreeBuildingType(tile.building.type)) {
          forceInstantConstructedBuilding(tile.building);
        } else {
  ...
  if (cityLifeMode) {
    newStats.money = Math.max(newStats.money, CITYLIFE_UNLIMITED_MONEY);
  }
...
export function placeBuilding(
  ...
  if (zone !== null) {
    ...
    if (size.width > 1 || size.height > 1) {
      ...
      if (cityLifeMode) {
        applyCityLifeBuildingOverrides(newGrid, x, y, size.width, size.height);
      }
    } else {
      ...
      if (cityLifeMode) {
        applyCityLifeBuildingOverrides(newGrid, x, y, 1, 1);
      }
    }
