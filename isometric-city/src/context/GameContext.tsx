...
export function GameProvider({
  ...
  const placeAtTile = useCallback((x: number, y: number, isRemote = false) => {
    ...
    setState((prev) => {
      ...
      if (!tile) return prev;
      if (!hasUnlimitedBudget && cost > 0 && prev.stats.money < cost) return prev;
      ...
      if (cityLifeOptions && cityLifeOptions.includes(tile.building.type)) return prev;
      if (!cityLifeOptions && building && tile.building.type === building) return prev;
      ...
      if (tool === 'subway') {
        ...
        return {
          ...
          stats: {
            ...nextState.stats,
            money: hasUnlimitedBudget ? nextState.stats.money : nextState.stats.money - cost,
          },
        };
      }
      ...
      } else if (cityLifeOptions) {
        nextState = placeCityLifeToolBuilding(prev, x, y, tool);
      } else if (building) {
